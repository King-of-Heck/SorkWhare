...
test('branding: HeckSoft + v1.3.0, no HeckNet, no stale versions', ()=>{
  const {__html:h}=loadApp();
  assert.match(h,/<title>SorkWhare Compare® v1\.3\.0 — HeckSoft<\/title>/);
  assert.match(h,/HeckSoft — a King of Heck Company™/);
  assert.doesNotMatch(h,/HeckNet/);
  assert.match(h,/<span class="ver">v1\.3\.0<\/span>/);
});

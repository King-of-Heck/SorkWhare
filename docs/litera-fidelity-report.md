...
| R1 formatting-only changes | rubric | Absent | M | Bold added to a word with otherwise identical text yields `summary.total=0` (row stays `equal`); the tool renders the bold but does not flag that formatting changed. Litera's desktop product detects formatting changes (with an opt-out); its LawVu integration does not. |
...
cleaner or more explicitly categorized result. It is **absent on the feature
surface** that makes Litera a full compare product: formatting-only change
detection (R1), footnote/header/comment comparison (R2), a real `.docx`
tracked-changes export (R4), and per-change accept/reject (R5). Change
categorization exists (R6) but without Litera's configurable rendering sets.
...
- **R1 — formatting-only changes not detected** (Litera desktop detects them; the
  LawVu integration does not — genuine vendor ambiguity).
